...
import { ContainerEdit } from '../../pages/containers/Edit'
...
const App = (props) => {
  const { signIn, signOut, currentUser, setupApp } = props
  ...
  if (!currentUser) {
    return (
      <>
        {header}
        <div>You need to sign in to use this website</div>
      </>
    )
  }
      ...
      <Router history={history}>
        <div>
          <Sidebar closed={menuClosed}>
            <Link onClick={() => setMenuClosed(true)} to={ROUTES.HOME}>
              <SidebarItem>Home</SidebarItem>
            </Link>
            <Link onClick={() => setMenuClosed(true)} to={ROUTES.RESOURCES}>
              <SidebarItem>Resources</SidebarItem>
            </Link>
          </Sidebar>
          <div className={styles.content}>
            <Switch>
              <Route path="/" component={ContainersList} exact />
              <Route path={ROUTES.CONTAINER_EDIT} exact>
                <ContainerEdit />
              </Route>
              <Route path={`${ROUTES.CONTAINER_EDIT}/:id`} exact>
                <ContainerEdit />
              </Route>
              ...
              <Route path={`${ROUTES.SCENES_EDIT}/:scenarioId`} component={SceneEditPage} exact />
              <Route path={`${ROUTES.SCENES_EDIT}/:scenarioId/:id`} component={SceneEditPage} />

              {/* Campaigns */}
              <Route path={ROUTES.CAMPAIGNS} component={CampaignsListPage} exact />
              <Route path={`${ROUTES.CAMPAIGNS_EDIT}`} component={CampaignEditPage} exact />
              <Route path={`${ROUTES.CAMPAIGNS}/:id`} component={CampaignPage} exact />
              <Route path={`${ROUTES.CAMPAIGNS_EDIT}/:id`} component={CampaignEditPage} />

              {/* Resources */}
              <Route path={`${ROUTES.RESOURCES}`} component={ResourcesPage} exact />
